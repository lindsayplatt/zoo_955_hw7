...
# This needs the `spatstat` library loaded
generate_clustered_pt_proc <- function(
...
generate_csr_pt_proc <- function(
  ...
  # A CSR process is synonymous with a Poisson process
  width <- Xmax - Xmin
  length <- Ymax - Ymin
  area <- length * width
  
  # First get lambda (the "intensity") or points per unit area
  lambda = Pointnum / area
  
  # Now generate a point pattern using the Poisson process
  set.seed(seed)
  pois.pp <- rpoispp(lambda = lambda, win = owin(c(Xmin,Xmax), c(Ymin,Ymax)))
  
  return(pois.pp)
}

plot_point_pattern <- function(point_pattern_object, random=FALSE, title = "") {
  
  # Plot the points and the cluster centers
  plot(point_pattern_object$x, point_pattern_object$y, main = title,
       xlim = point_pattern_object$window$xrange, 
       ylim = point_pattern_object$window$yrange)
  
  # Add the red cluster dots if it isn't random
  if(!random) {
    centers=matrix(data=c(-25,-25,25,25,-25,25,-25,25),nrow=4,ncol=2)
